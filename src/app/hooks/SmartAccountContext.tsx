...
const transportUrl = (chain: Chain) => `https://api.pimlico.io/v2/${chain.id}/rpc?apikey=${process.env.NEXT_PUBLIC_PIMLICO_API_KEY}`;
  ...
  useEffect(() => {
    ...
    const createSmartWallet = async (eoa: ConnectedWallet) => {
      ...
      const publicClient = createPublicClient({
        chain: polygonAmoy, // Replace this with the chain of your app
        transport: http(),
      });
      ...
      const smartAccountClient = createSmartAccountClient({
        account: safeAccount,
        entryPoint: ENTRYPOINT_ADDRESS_V07,
        chain: polygonAmoy, // Replace this with the chain for your app
        bundlerTransport: http(transportUrl(polygonAmoy)),
        middleware: {
